...
linuxType=`cat $1 | grep -m1  'ID=' | cut -d = -f 2` 
...
ubuntuVersion=`cat $1 | grep 'VERSION_ID' | cut -b 13-14`
if test $ubuntuVersion = $requiredUbuntuVersion || $ubuntuVersion ge $requiredUbuntuVersion
   then
